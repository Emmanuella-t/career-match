"""Persistence store protocol, in-memory implementation, and Supabase factory."""

from __future__ import annotations

import os
import threading
from datetime import UTC, datetime
from typing import Any, Protocol
from uuid import UUID, uuid4

from career_match.persistence.errors import (
    PersistenceError,
    PersistenceNotConfiguredError,
    PersistenceUnavailableError,
    RecordNotFoundError,
)
from career_match.persistence.schemas import (
    MatchAnalysisCreate,
    MatchAnalysisRecord,
    ProfileUpsert,
    ResumeCreate,
    ResumeRecord,
    ResumeUpdate,
    SavedJobCreate,
    SavedJobRecord,
    SavedJobUpdate,
    UserProfile,
)


def _utcnow() -> datetime:
    return datetime.now(UTC)


class PersistenceStore(Protocol):
    """User-scoped persistence operations."""

    def upsert_profile(self, clerk_user_id: str, payload: ProfileUpsert) -> UserProfile: ...

    def get_profile(self, clerk_user_id: str) -> UserProfile | None: ...

    def create_resume(self, clerk_user_id: str, payload: ResumeCreate) -> ResumeRecord: ...

    def list_resumes(self, clerk_user_id: str) -> list[ResumeRecord]: ...

    def get_resume(self, clerk_user_id: str, resume_id: UUID) -> ResumeRecord: ...

    def update_resume(
        self, clerk_user_id: str, resume_id: UUID, payload: ResumeUpdate
    ) -> ResumeRecord: ...

    def delete_resume(self, clerk_user_id: str, resume_id: UUID) -> None: ...

    def create_match(
        self, clerk_user_id: str, payload: MatchAnalysisCreate
    ) -> MatchAnalysisRecord: ...

    def list_matches(self, clerk_user_id: str) -> list[MatchAnalysisRecord]: ...

    def get_match(self, clerk_user_id: str, match_id: UUID) -> MatchAnalysisRecord: ...

    def delete_match(self, clerk_user_id: str, match_id: UUID) -> None: ...

    def create_job(self, clerk_user_id: str, payload: SavedJobCreate) -> SavedJobRecord: ...

    def list_jobs(self, clerk_user_id: str) -> list[SavedJobRecord]: ...

    def get_job(self, clerk_user_id: str, job_id: UUID) -> SavedJobRecord: ...

    def update_job(
        self, clerk_user_id: str, job_id: UUID, payload: SavedJobUpdate
    ) -> SavedJobRecord: ...

    def delete_job(self, clerk_user_id: str, job_id: UUID) -> None: ...


class InMemoryPersistenceStore:
    """Thread-safe store for unit tests (no network)."""

    def __init__(self) -> None:
        self._lock = threading.Lock()
        self.profiles: dict[str, UserProfile] = {}
        self.resumes: dict[UUID, ResumeRecord] = {}
        self.matches: dict[UUID, MatchAnalysisRecord] = {}
        self.jobs: dict[UUID, SavedJobRecord] = {}

    def upsert_profile(self, clerk_user_id: str, payload: ProfileUpsert) -> UserProfile:
        with self._lock:
            existing = self.profiles.get(clerk_user_id)
            now = _utcnow()
            if existing is None:
                record = UserProfile(
                    id=uuid4(),
                    clerk_user_id=clerk_user_id,
                    email=payload.email,
                    display_name=payload.display_name,
                    created_at=now,
                    updated_at=now,
                )
            else:
                record = existing.model_copy(
                    update={
                        "email": payload.email
                        if payload.email is not None
                        else existing.email,
                        "display_name": payload.display_name
                        if payload.display_name is not None
                        else existing.display_name,
                        "updated_at": now,
                    }
                )
            self.profiles[clerk_user_id] = record
            return record

    def get_profile(self, clerk_user_id: str) -> UserProfile | None:
        with self._lock:
            return self.profiles.get(clerk_user_id)

    def create_resume(self, clerk_user_id: str, payload: ResumeCreate) -> ResumeRecord:
        with self._lock:
            now = _utcnow()
            record = ResumeRecord(
                id=uuid4(),
                clerk_user_id=clerk_user_id,
                name=payload.name,
                resume_text=payload.resume_text,
                created_at=now,
                updated_at=now,
            )
            self.resumes[record.id] = record
            return record

    def list_resumes(self, clerk_user_id: str) -> list[ResumeRecord]:
        with self._lock:
            rows = [r for r in self.resumes.values() if r.clerk_user_id == clerk_user_id]
            return sorted(rows, key=lambda r: r.updated_at, reverse=True)

    def get_resume(self, clerk_user_id: str, resume_id: UUID) -> ResumeRecord:
        with self._lock:
            record = self.resumes.get(resume_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("resume not found")
            return record

    def update_resume(
        self, clerk_user_id: str, resume_id: UUID, payload: ResumeUpdate
    ) -> ResumeRecord:
        with self._lock:
            record = self.resumes.get(resume_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("resume not found")
            updated = record.model_copy(
                update={
                    "name": payload.name if payload.name is not None else record.name,
                    "resume_text": payload.resume_text
                    if payload.resume_text is not None
                    else record.resume_text,
                    "updated_at": _utcnow(),
                }
            )
            self.resumes[resume_id] = updated
            return updated

    def delete_resume(self, clerk_user_id: str, resume_id: UUID) -> None:
        with self._lock:
            record = self.resumes.get(resume_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("resume not found")
            del self.resumes[resume_id]
            for match_id, match in list(self.matches.items()):
                if match.resume_id == resume_id:
                    self.matches[match_id] = match.model_copy(update={"resume_id": None})

    def create_match(
        self, clerk_user_id: str, payload: MatchAnalysisCreate
    ) -> MatchAnalysisRecord:
        with self._lock:
            if payload.resume_id is not None:
                resume = self.resumes.get(payload.resume_id)
                if resume is None or resume.clerk_user_id != clerk_user_id:
                    raise RecordNotFoundError("resume not found")
            record = MatchAnalysisRecord(
                id=uuid4(),
                clerk_user_id=clerk_user_id,
                resume_id=payload.resume_id,
                job_title=payload.job_title,
                company=payload.company,
                job_description=payload.job_description,
                matcher=payload.matcher,
                matcher_version=payload.matcher_version,
                overall_score=payload.overall_score,
                matched_skills=list(payload.matched_skills),
                missing_skills=list(payload.missing_skills),
                weak_or_negated_skills=list(payload.weak_or_negated_skills),
                semantic_score=payload.semantic_score,
                tfidf_score=payload.tfidf_score,
                skill_overlap_score=payload.skill_overlap_score,
                disclaimer=payload.disclaimer,
                created_at=_utcnow(),
            )
            self.matches[record.id] = record
            return record

    def list_matches(self, clerk_user_id: str) -> list[MatchAnalysisRecord]:
        with self._lock:
            rows = [m for m in self.matches.values() if m.clerk_user_id == clerk_user_id]
            return sorted(rows, key=lambda m: m.created_at, reverse=True)

    def get_match(self, clerk_user_id: str, match_id: UUID) -> MatchAnalysisRecord:
        with self._lock:
            record = self.matches.get(match_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("match not found")
            return record

    def delete_match(self, clerk_user_id: str, match_id: UUID) -> None:
        with self._lock:
            record = self.matches.get(match_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("match not found")
            del self.matches[match_id]

    def create_job(self, clerk_user_id: str, payload: SavedJobCreate) -> SavedJobRecord:
        with self._lock:
            now = _utcnow()
            record = SavedJobRecord(
                id=uuid4(),
                clerk_user_id=clerk_user_id,
                title=payload.title,
                company=payload.company,
                job_description=payload.job_description,
                source_url=payload.source_url,
                notes=payload.notes,
                created_at=now,
                updated_at=now,
            )
            self.jobs[record.id] = record
            return record

    def list_jobs(self, clerk_user_id: str) -> list[SavedJobRecord]:
        with self._lock:
            rows = [j for j in self.jobs.values() if j.clerk_user_id == clerk_user_id]
            return sorted(rows, key=lambda j: j.updated_at, reverse=True)

    def get_job(self, clerk_user_id: str, job_id: UUID) -> SavedJobRecord:
        with self._lock:
            record = self.jobs.get(job_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("job not found")
            return record

    def update_job(
        self, clerk_user_id: str, job_id: UUID, payload: SavedJobUpdate
    ) -> SavedJobRecord:
        with self._lock:
            record = self.jobs.get(job_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("job not found")
            data = record.model_dump()
            for field in ("title", "company", "job_description", "source_url", "notes"):
                value = getattr(payload, field)
                if value is not None:
                    data[field] = value
            data["updated_at"] = _utcnow()
            updated = SavedJobRecord(**data)
            self.jobs[job_id] = updated
            return updated

    def delete_job(self, clerk_user_id: str, job_id: UUID) -> None:
        with self._lock:
            record = self.jobs.get(job_id)
            if record is None or record.clerk_user_id != clerk_user_id:
                raise RecordNotFoundError("job not found")
            del self.jobs[job_id]


def _parse_uuid(value: Any) -> UUID:
    return value if isinstance(value, UUID) else UUID(str(value))


def _parse_dt(value: Any) -> datetime:
    if isinstance(value, datetime):
        return value if value.tzinfo else value.replace(tzinfo=UTC)
    return datetime.fromisoformat(str(value).replace("Z", "+00:00"))


def _as_str_list(value: Any) -> list[str]:
    if value is None:
        return []
    if isinstance(value, list):
        return [str(item) for item in value]
    return []


class SupabasePersistenceStore:
    """Supabase Postgres store using the service-role client (backend only)."""

    def __init__(self, client: Any) -> None:
        self._client = client

    def _table(self, name: str) -> Any:
        return self._client.table(name)

    def _raise_unavailable(self, exc: Exception) -> None:
        raise PersistenceUnavailableError("persistence service unavailable") from exc

    def upsert_profile(self, clerk_user_id: str, payload: ProfileUpsert) -> UserProfile:
        row = {
            "clerk_user_id": clerk_user_id,
            "email": payload.email,
            "display_name": payload.display_name,
            "updated_at": _utcnow().isoformat(),
        }
        try:
            response = (
                self._table("user_profiles")
                .upsert(row, on_conflict="clerk_user_id")
                .execute()
            )
            data = (response.data or [None])[0]
            if not data:
                raise PersistenceUnavailableError("persistence service unavailable")
            return self._profile_from_row(data)
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def get_profile(self, clerk_user_id: str) -> UserProfile | None:
        try:
            response = (
                self._table("user_profiles")
                .select("*")
                .eq("clerk_user_id", clerk_user_id)
                .limit(1)
                .execute()
            )
            rows = response.data or []
            if not rows:
                return None
            return self._profile_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def create_resume(self, clerk_user_id: str, payload: ResumeCreate) -> ResumeRecord:
        row = {
            "clerk_user_id": clerk_user_id,
            "name": payload.name,
            "resume_text": payload.resume_text,
        }
        try:
            response = self._table("resumes").insert(row).execute()
            data = (response.data or [None])[0]
            if not data:
                raise PersistenceUnavailableError("persistence service unavailable")
            return self._resume_from_row(data)
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def list_resumes(self, clerk_user_id: str) -> list[ResumeRecord]:
        try:
            response = (
                self._table("resumes")
                .select("*")
                .eq("clerk_user_id", clerk_user_id)
                .order("updated_at", desc=True)
                .execute()
            )
            return [self._resume_from_row(row) for row in (response.data or [])]
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def get_resume(self, clerk_user_id: str, resume_id: UUID) -> ResumeRecord:
        try:
            response = (
                self._table("resumes")
                .select("*")
                .eq("id", str(resume_id))
                .eq("clerk_user_id", clerk_user_id)
                .limit(1)
                .execute()
            )
            rows = response.data or []
            if not rows:
                raise RecordNotFoundError("resume not found")
            return self._resume_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def update_resume(
        self, clerk_user_id: str, resume_id: UUID, payload: ResumeUpdate
    ) -> ResumeRecord:
        updates: dict[str, Any] = {"updated_at": _utcnow().isoformat()}
        if payload.name is not None:
            updates["name"] = payload.name
        if payload.resume_text is not None:
            updates["resume_text"] = payload.resume_text
        try:
            response = (
                self._table("resumes")
                .update(updates)
                .eq("id", str(resume_id))
                .eq("clerk_user_id", clerk_user_id)
                .execute()
            )
            rows = response.data or []
            if not rows:
                raise RecordNotFoundError("resume not found")
            return self._resume_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def delete_resume(self, clerk_user_id: str, resume_id: UUID) -> None:
        try:
            response = (
                self._table("resumes")
                .delete()
                .eq("id", str(resume_id))
                .eq("clerk_user_id", clerk_user_id)
                .execute()
            )
            if not (response.data or []):
                raise RecordNotFoundError("resume not found")
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def create_match(
        self, clerk_user_id: str, payload: MatchAnalysisCreate
    ) -> MatchAnalysisRecord:
        if payload.resume_id is not None:
            self.get_resume(clerk_user_id, payload.resume_id)
        row = {
            "clerk_user_id": clerk_user_id,
            "resume_id": str(payload.resume_id) if payload.resume_id else None,
            "job_title": payload.job_title,
            "company": payload.company,
            "job_description": payload.job_description,
            "matcher": payload.matcher,
            "matcher_version": payload.matcher_version,
            "overall_score": payload.overall_score,
            "matched_skills": list(payload.matched_skills),
            "missing_skills": list(payload.missing_skills),
            "weak_or_negated_skills": list(payload.weak_or_negated_skills),
            "semantic_score": payload.semantic_score,
            "tfidf_score": payload.tfidf_score,
            "skill_overlap_score": payload.skill_overlap_score,
            "disclaimer": payload.disclaimer,
        }
        try:
            response = self._table("match_analyses").insert(row).execute()
            data = (response.data or [None])[0]
            if not data:
                raise PersistenceUnavailableError("persistence service unavailable")
            return self._match_from_row(data)
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def list_matches(self, clerk_user_id: str) -> list[MatchAnalysisRecord]:
        try:
            response = (
                self._table("match_analyses")
                .select("*")
                .eq("clerk_user_id", clerk_user_id)
                .order("created_at", desc=True)
                .execute()
            )
            return [self._match_from_row(row) for row in (response.data or [])]
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def get_match(self, clerk_user_id: str, match_id: UUID) -> MatchAnalysisRecord:
        try:
            response = (
                self._table("match_analyses")
                .select("*")
                .eq("id", str(match_id))
                .eq("clerk_user_id", clerk_user_id)
                .limit(1)
                .execute()
            )
            rows = response.data or []
            if not rows:
                raise RecordNotFoundError("match not found")
            return self._match_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def delete_match(self, clerk_user_id: str, match_id: UUID) -> None:
        try:
            response = (
                self._table("match_analyses")
                .delete()
                .eq("id", str(match_id))
                .eq("clerk_user_id", clerk_user_id)
                .execute()
            )
            if not (response.data or []):
                raise RecordNotFoundError("match not found")
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def create_job(self, clerk_user_id: str, payload: SavedJobCreate) -> SavedJobRecord:
        row = {
            "clerk_user_id": clerk_user_id,
            "title": payload.title,
            "company": payload.company,
            "job_description": payload.job_description,
            "source_url": payload.source_url,
            "notes": payload.notes,
        }
        try:
            response = self._table("saved_jobs").insert(row).execute()
            data = (response.data or [None])[0]
            if not data:
                raise PersistenceUnavailableError("persistence service unavailable")
            return self._job_from_row(data)
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def list_jobs(self, clerk_user_id: str) -> list[SavedJobRecord]:
        try:
            response = (
                self._table("saved_jobs")
                .select("*")
                .eq("clerk_user_id", clerk_user_id)
                .order("updated_at", desc=True)
                .execute()
            )
            return [self._job_from_row(row) for row in (response.data or [])]
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def get_job(self, clerk_user_id: str, job_id: UUID) -> SavedJobRecord:
        try:
            response = (
                self._table("saved_jobs")
                .select("*")
                .eq("id", str(job_id))
                .eq("clerk_user_id", clerk_user_id)
                .limit(1)
                .execute()
            )
            rows = response.data or []
            if not rows:
                raise RecordNotFoundError("job not found")
            return self._job_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def update_job(
        self, clerk_user_id: str, job_id: UUID, payload: SavedJobUpdate
    ) -> SavedJobRecord:
        updates: dict[str, Any] = {"updated_at": _utcnow().isoformat()}
        for field in ("title", "company", "job_description", "source_url", "notes"):
            value = getattr(payload, field)
            if value is not None:
                updates[field] = value
        try:
            response = (
                self._table("saved_jobs")
                .update(updates)
                .eq("id", str(job_id))
                .eq("clerk_user_id", clerk_user_id)
                .execute()
            )
            rows = response.data or []
            if not rows:
                raise RecordNotFoundError("job not found")
            return self._job_from_row(rows[0])
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    def delete_job(self, clerk_user_id: str, job_id: UUID) -> None:
        try:
            response = (
                self._table("saved_jobs")
                .delete()
                .eq("id", str(job_id))
                .eq("clerk_user_id", clerk_user_id)
                .execute()
            )
            if not (response.data or []):
                raise RecordNotFoundError("job not found")
        except PersistenceError:
            raise
        except Exception as exc:  # noqa: BLE001
            self._raise_unavailable(exc)
            raise

    @staticmethod
    def _profile_from_row(row: dict[str, Any]) -> UserProfile:
        return UserProfile(
            id=_parse_uuid(row["id"]),
            clerk_user_id=str(row["clerk_user_id"]),
            email=row.get("email"),
            display_name=row.get("display_name"),
            created_at=_parse_dt(row["created_at"]),
            updated_at=_parse_dt(row["updated_at"]),
        )

    @staticmethod
    def _resume_from_row(row: dict[str, Any]) -> ResumeRecord:
        return ResumeRecord(
            id=_parse_uuid(row["id"]),
            clerk_user_id=str(row["clerk_user_id"]),
            name=str(row["name"]),
            resume_text=str(row["resume_text"]),
            created_at=_parse_dt(row["created_at"]),
            updated_at=_parse_dt(row["updated_at"]),
        )

    @staticmethod
    def _match_from_row(row: dict[str, Any]) -> MatchAnalysisRecord:
        resume_id = row.get("resume_id")
        return MatchAnalysisRecord(
            id=_parse_uuid(row["id"]),
            clerk_user_id=str(row["clerk_user_id"]),
            resume_id=_parse_uuid(resume_id) if resume_id else None,
            job_title=row.get("job_title"),
            company=row.get("company"),
            job_description=str(row["job_description"]),
            matcher=str(row["matcher"]),
            matcher_version=row.get("matcher_version"),
            overall_score=float(row["overall_score"]),
            matched_skills=_as_str_list(row.get("matched_skills")),
            missing_skills=_as_str_list(row.get("missing_skills")),
            weak_or_negated_skills=_as_str_list(row.get("weak_or_negated_skills")),
            semantic_score=row.get("semantic_score"),
            tfidf_score=row.get("tfidf_score"),
            skill_overlap_score=row.get("skill_overlap_score"),
            disclaimer=row.get("disclaimer"),
            created_at=_parse_dt(row["created_at"]),
        )

    @staticmethod
    def _job_from_row(row: dict[str, Any]) -> SavedJobRecord:
        return SavedJobRecord(
            id=_parse_uuid(row["id"]),
            clerk_user_id=str(row["clerk_user_id"]),
            title=str(row["title"]),
            company=row.get("company"),
            job_description=str(row["job_description"]),
            source_url=row.get("source_url"),
            notes=row.get("notes"),
            created_at=_parse_dt(row["created_at"]),
            updated_at=_parse_dt(row["updated_at"]),
        )


def supabase_configured() -> bool:
    url = os.environ.get("SUPABASE_URL", "").strip()
    key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    return bool(url and key)


def create_supabase_client() -> Any:
    """Create a Supabase client from environment (service role)."""
    url = os.environ.get("SUPABASE_URL", "").strip()
    key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    if not url or not key:
        raise PersistenceNotConfiguredError(
            "SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required for persistence"
        )
    try:
        from supabase import create_client
    except ImportError as exc:  # pragma: no cover
        raise PersistenceNotConfiguredError(
            "supabase package is not installed; pip install -e '.[api]'"
        ) from exc
    return create_client(url, key)


def get_persistence_store(override: PersistenceStore | None = None) -> PersistenceStore:
    """Return the configured store, or ``override`` when provided (tests)."""
    if override is not None:
        return override
    client = create_supabase_client()
    return SupabasePersistenceStore(client)
